...
# Global Subtotal Calculation (needed for both Cart and Rewards tabs)
...
with tab_menu:
    ...
    for item in MENU[selected_category]:
        with st.container():
            # Adjusted ratio to give text more room, button sits nicely on the right
            ...
            with col_btn:
                # Add a little top margin so the button aligns better with the content block
        ...
        with st.expander("💳 Credit / Debit Card"):
            with st.form("cc_form"):
                ...
                if st.form_submit_button("Submit Order", type="primary", use_container_width=True, disabled=submit_disabled):
                    # Finalize transaction in session state
